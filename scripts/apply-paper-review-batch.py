#!/usr/bin/env python3
"""Apply a staged batch of reviewed paper IDs into papers/reviewed-papers.json."""
...
def parse_review_ids_json(raw: str) -> list[str]:
    ...
    if not text:
        raise ValueError("review_ids_json is required")
    ...
    if not out:
        raise ValueError("review_ids_json does not contain any paper ids")
...
def main() -> int:
    ...
    parser.add_argument("--review-ids-json", required=True)
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    try:
        review_ids = parse_review_ids_json(args.review_ids_json)
        known_paper_ids = collect_known_paper_ids(repo_root)

        unknown = [paper_id for paper_id in review_ids if paper_id not in known_paper_ids]
        if unknown:
            sample = ", ".join(unknown[:10])
            suffix = "" if len(unknown) <= 10 else f" (+{len(unknown) - 10} more)"
            raise ValueError(f"Unknown paper ids in review_ids_json: {sample}{suffix}")
        ...
        merged_reviews, added_ids = apply_review_batch(existing_reviews, review_ids, now_iso)

        if not added_ids:
            raise ValueError("No new paper ids were added; every id is already marked reviewed")
        ...
        print(f"Applied reviewed paper batch entries: {len(added_ids)}")
        print(f"Total permanently reviewed papers: {len(merged_reviews)}")
        print(f"Output file: {reviewed_path}")
        return 0
